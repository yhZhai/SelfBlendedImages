...
def eval(model, criterion, val_loader, writer, epoch: int, args):
    ...
    for step, data in metric_logger.log_every(val_loader, header=f"[val {epoch}]"):
        ...
        output_list += [1 - o for o in output.softmax(1)[:, 0].cpu().data.numpy().tolist()]
        target_list += [int(t > 0) for t in target.cpu().data.numpy().tolist()]
...
def main(args):
    ...
    model = Detector(args, num_classes=7)
    model = model.to("cuda")

    state_dict = "FFc23.tar"
    state_dict = torch.load(state_dict, map_location="cpu")["model"]
    state_dict.pop("net._fc.weight")
    state_dict.pop("net._fc.bias")
