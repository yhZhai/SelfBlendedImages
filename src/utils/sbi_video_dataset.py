...
class SBIVideoDataset(Dataset):
    def __init__(
        ...
        label_list = [
            ...
            if os.path.isfile(
                image_list[i].replace("/frames/", path_lm).replace(".png", ".npy")
            )
            and os.path.isfile(
        ...
        self.image_dict = self._get_datalist()
    ...
    def __getitem__(self, idx):
        ...
        while flag:
            try:
                ...
                bboxes = np.load(
                    filename.replace(".png", ".npy").replace("/frames/", "/retina/")
                )[:2]
                iou_max = -1
                ...
                if self.phase == "train":
                    transformed = self.transforms(
                        image=img_f.astype("uint8"), image1=img_r.astype("uint8")
                    )
...
if __name__ == "__main__":
    patch_size = 32
    dataset = SBIVideoDataset(phase="train", patch_size=patch_size)
