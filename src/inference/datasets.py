...
def init_ff(dataset="all", phase="test"):
    ...
    for fake in fakes:
        ...
        folder_list += [
            i for i in folder_list_all if os.path.basename(i)[:3] in filelist
        ]
...
def init_dfd():
    ...
    fake_path = (
        "data/FaceForensics++/manipulated_sequences/DeepFakeDetection/raw/videos/*.mp4"
    )
...
def init_dfdcp(phase="test"):
    ...
    fol_lab_list_all = [
        [
            f"data/DFDCP/{k.split('/')[0]}/videos/{k.split('/')[-1]}",
            df[k]["label"] == "fake",
        ]
        for k in df
        if df[k]["set"] == phase_integrated[phase]
    ]
    name2lab = {
        os.path.basename(fol_lab_list_all[i][0]): fol_lab_list_all[i][1]
        for i in range(len(fol_lab_list_all))
    }
    fol_list_all = [f[0] for f in fol_lab_list_all]
    fol_list_all = [os.path.basename(p) for p in fol_list_all]
    folder_list = glob("data/DFDCP/method_*/videos/*/*/*.mp4") + glob(
        "data/DFDCP/original_videos/videos/*/*.mp4"
    )
