    ...
    def forward(self, cos_theta, target):
        ...
        if self.t > 1:
            ...
            support_vecs_mask = (1 - index) * torch.lt(
                torch.masked_select(phi_theta, index)
                .view(-1, 1)
                .repeat(1, h_theta.shape[1])
                - cos_theta,
                0,
            )
        ...
        return focal_loss(
            F.cross_entropy(self.s * output, target, reduction="none"), self.gamma
        )
...
class AMSoftmax_OHEM(nn.Module):
    ...
    def forward(self, cos_theta, target):
        ...
        return focal_loss(
            F.cross_entropy(self.s * output2, target2, reduction="none"), self.gamma
        )
