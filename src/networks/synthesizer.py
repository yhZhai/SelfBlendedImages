...
def generate_random_mask(mask, res=256):
    ...
    newmask[
        max(center_x - randwl, 0) : min(center_x + randwr, res - 1),
...
def blend_fake_to_real(
    realimg, real_lmk, fakeimg, fakemask, fake_lmk, deformed_fakemask, type, mag
):
    # source: fake image
    # taret: real image
    ...
    aligned_src = cv2.warpAffine(
        fakeimg, aff_param, (W, H), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REFLECT
    )
    ...
    if tgt_mask.mean() < 0.005 or src_crop.max() == 0:
        ...
        if type == 0:
            out_blend, a_mask = alpha_blend_fea(
                src_crop, realimg, tgt_mask, featherAmount=0.2 * np.random.rand()
            )
        elif type == 1:
            b_mask = (tgt_mask * 255).astype(np.uint8)
            l, t, w, h = cv2.boundingRect(b_mask)
            center = (int(l + w / 2), int(t + h / 2))
            out_blend = cv2.seamlessClone(
                src_crop, realimg, b_mask, center, cv2.NORMAL_CLONE
            )
        else:
...
class synthesizer(nn.Module):
    def __init__(self):
        ...
        self.netG = TransferModel(
            "xception", num_region=10, num_type=4, num_mag=1, inc=6
        )
    ...
    def parse(self, img, reg, real_lmk, fakemask):
        ...
        if reg == 0:
            ...
            mask = (
                remove_eyes(img, five_key, "b")
                + remove_nose(img, five_key)
                + remove_mouth(img, five_key)
            )
        else:
    ...
    def calculate(self, logits):
        if logits.shape[1] != 1:
            ...
            selected_log_prob = log_prob.gather(
                1, self.get_variable(action, requires_grad=False)
            )
        else:
        ...
        for i in range(img.shape[0]):
            ...
            if label[i] == 0 and type[i] != 3:
                mask = self.parse(
                    fake_imgcp,
                    reg[i],
                    fake_lmk[i, 0].cpu().numpy(),
                    fake_mask[i, 0].cpu().numpy(),
                )
                newimg, newmask = blend_fake_to_real(
                    imgcp,
                    real_lmk[i, 0].cpu().numpy(),
                    fake_imgcp,
                    fake_mask.cpu().numpy(),
                    fake_lmk[i, 0].cpu().numpy(),
                    mask,
                ...
                newimg = self.transforms(
                    Image.fromarray(np.array(newimg, dtype=np.uint8))
                )
                ...
                newimg = self.transforms(
                    Image.fromarray(np.array((imgcp + 1) / 2 * 255, dtype=np.uint8))
                )
